...
<%
	...
	if ( request.getAttribute("trainer") != null) {
		trainers = (ArrayList<User>) request.getAttribute("trainer");
	}
%>
<%
...
<div class="<%= classNameContent%>" <%= styleContent%> style="overflow-x: inherit;">
  <div class="<%= classNameContainer%>">
  	<div class="alert1 alert alert-primary" style="margin-left: -14px;"> 
		...
		<strong class="lb_name" style="">Reopen <%= classAdd.getName()%></strong>
		</div>
			  <div>
			  	<%
        ...
        <div class="card mb-3" style="padding-top: 10px; margin-top: 0px;padding-bottom: 44px;">
             <form id="add-post" action="<%= request.getContextPath()%>/classes/reopen" method="POST" >
              	<div style="height: 5%"></div>
              	
              	 <div class="col-sm-4"  style="float: left"> 
	              	<input class="form-control"  type="text" name="id" value="<%= classAdd.getClassId()%>" style="display: none;" />
	              		
	              	<div class="form-group">
		              	<label class="required" ><strong>Name:</strong>&nbsp;</label>
		              	<input class="form-control" id="txtName"  type="text" name="name" value="<%= classAdd.getName()%>" disabled placeholder="Class name" />
	              		<span id="spnNameStatus"></span>
					</div>
	              	
	              	<div class="form-group">
		              	<label class="required" ><strong>Course:</strong>&nbsp;</label>
	  					<input  class="form-control"  type="text" name="course" disabled value="<%= classAdd.getNameCourse()%>"> 
	              	</div>
	              	<div class="form-group">
	              		<label class='required' ><strong>Trainer:</strong>&nbsp;:</label>
	              		<select name='trainer' id='trainerSelect' style='width: 100%'> 
	              			<option onclick='showRoom();' value='0'>Select trainer</option> 
	              			<%
	              			for (User trainer : trainers) {
	              			%>
	              			<option onclick='showRoom();' value="<%= trainer.getUserId()%>"> <%= trainer.getUsername() %>( <%= trainer.getAbility().getExperience() %>years experience)</option>
	              			<%
	              			} 
	              			%>
	              		</select>
	              		<span id='spnTrainerStatus'></span> 
	              	</div>
	             ...
	             <div class="col-sm-4"  style="float: left">
	              ...
	              <div style="height: 5%"></div>
	             	...
	             	<div style="text-align:center;margin-top:15px;">
	           		<input id="btnSubmit" class="btn btn-primary" style="height: 40px; width:100px; font-size: 17px;" type="submit" name="submit" value="Open" />
